...
namespace ClassToksV2.ViewModels
{
    ...
    public class TokInfoViewModel : BaseViewModel
    {
        ...
        public string[] options = new string[] { "+ Add to Class Set", "Edit", "Delete", "Share" };
        public WaitingView Loader { get; set; }

        public TokInfoViewModel(ClassTokXF item)
        {
...
#endif
        {
            ...
            string selected = "";
            selected = await App.Current.MainPage.DisplayActionSheet("Actions", "Cancel", null, options);

            if (selected == "Edit")
            {
                App.TokParameter.IsAddMode = false;
            ...
            else if (selected == "Delete")
            {
                try
                {
                    ...
                    if (accept)
                    {
                        ...
                        await ClassService.Instance.DeleteClassToksAsync(Item.Id, Item.PartitionKey);
                        App.Toks.Items.Remove(App.TokParameter);// TODO: don't allow screen to be touched until tok removed. Does not like the top tab switching
